...
- **Found:** `(labels, syndromes)` pairs are duplicate-free **within every one
  ...
  *(Correction: an earlier pass pooled all 7 files together and reported
  "3,696 patterns under both labels" as one global number. That number mixes
  physically different experiments and undercounts the per-file effect above —
  scoping the check to one experiment/file at a time, as done here, is the
  right question and the more useful finding.)*
